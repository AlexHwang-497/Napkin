...
const PortfolioPost = ({ post, setCurrentId }) => {
		...
		Promise.all(
			...
			Promise.all(results.map(res => res.json()))
				.then(stocks => {
					...
					console.log(
						'resultsInside.regDivData',
						divData.filter(user => user.symbol === 'SPY')
					);
	...
	fetch(
		'https://financialmodelingprep.com/api/v3/historical-price-full/stock_dividend/AAPL?apikey=f69c6a774b0cfb6186868a361929fd36'
	).then(res => res.json());
